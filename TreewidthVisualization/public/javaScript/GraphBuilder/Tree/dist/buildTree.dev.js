...
function setColorNodes(line) {
  var numberOfbag = line[1]; //String

  bagIds.push(numberOfbag);
  var vertices = line.indexOf('') === -1 ? line.slice(2, line.length) : line.slice(2, line.length - 1);
...
function setBagDependencies(lines, isFromServer) {
  if (isFromServer) lines = lines.split(/\r?\n/);
      ...
      switch (line[0]) {
        ...
        case 'b':
          if (line.length <= 2) {
            ...
            var message = 'Tried to define a bag, but this is not a bag on textline: ' + lineNumber + '.' + '</br>' + 'Be sure to follow the bag format: ' + '"<b><span style=color:blue> b </span> <span style=color:red> id </span> <span style=color:green> V_k" </span> </b>' + ', where id is <span style=color:red> the bag id </span> and <span style=color:green> V_k </span> ' + 'a <span style=color:green> list of nodes</span> (can be empty) of the  <span style=color:blue> bag </span> b. ';
            throw new Error(message);
          }

          var size = isFromServer ? line.length - 2 : line.length - 3;
        ...
        default:
          ...
          if (line.length < 2) {
            // <= because of empty last index
            var _message = 'Build Tree: Cannot create a loop edge on a node in line:' + lineNumber + '.  line code is ' + line;
...
function handleTreeLayout(layout, rePoseNodes) {
  ...
  if (layout !== 'preset') {
    ...
    if ($('#bubbleOptions')[0].children[1].value === 'active') drawConstructBubble();else if ($('#bubbleOptions')[0].children[2].value === 'active') drawFullBubble(); // child 0,none, does not need a query

    if (!rePoseNodes) treeLayoutClock = treeLayoutClock.getTime; // cr.animate('queue', false); 
  }
}
/**
...
function setTreeLayout(layout) {
  ...
  var layoutName = layout === 'preset' ? layout : layout.substring(2, layout.length);
  var layoutType = layout[0];
  var newLayout;
  ...
  } else if (layout === 'BTB') newLayout = treeLayout.breadthfirstBiggestBag(); //layout runs on all nodes (some layout can only this)
  else if (layoutType === 'a' || layout === 'preset') {
      newLayout = cr.makeLayout(treeLayout.layouts[layoutName]); //layout runs on bags/construct (not all layouts can do this)
