...
cy.on('tap', 'node', function (evt) {
  ...
  if (evt.originalEvent.shiftKey) {
    ...
    cy.startBatch();
    cy.nodes("[id = \"".concat(text, "\"]")).toggleClass('notTarget', false);
    cy.endBatch();
    cr.startBatch();
    cr.nodes('.construct').toggleClass('notTarget', false);
    cr.nodes('.tree').filter("[displayedText = \"".concat(text, "\"]")).toggleClass('notTarget', false);
    cr.edges('.tree').filter("[nodeGroup = \"".concat(text, "\"]")).toggleClass('notTarget', false);
    cr.endBatch();
  } else {
    //single node allowed
    cy.startBatch();
...
cr.on('tap', 'node.construct', function (evt) {
  ...
  if (evt.originalEvent.shiftKey) {
    ...
    bagNodes = bagNodes.map(function (m) {
      return m.data('displayedText');
    });
    bagNodes.forEach(function (node) {
      cy.nodes("[id = \"".concat(node, "\"]")).toggleClass('notTarget', false);
    });
    cy.endBatch();
    cr.startBatch();
    cr.nodes('.construct').filter("[id = \"".concat(text, "\"]")).toggleClass('notTarget', false);
    cr.nodes('.tree').filter("[bag = \"".concat(text, "\"]")).toggleClass('notTarget', false);
    cr.endBatch();
  } else {
    //no shift, only one bag is selected
    cy.startBatch(); //show the nodes of tree in graph in color
...
cr.on('tap', 'node.tree', function (evt) {
  ...
  if (evt.originalEvent.shiftKey) {
    ...
    //Visual the group content of the selcted node 
    cy.startBatch();
    cy.nodes().toggleClass('notTarget', true);
    cy.nodes("[id = \"".concat(text, "\"]")).toggleClass('notTarget', false);
    cy.endBatch();
...
function disableExtendingButtons(evt) {
  var e = evt.originalEvent.target.localName;
...
function checkArea(evt) {
  var e = evt.originalEvent.target.localName;
  if (e === 'button' || e === 'span' || e === 'i') return true;
}