...
function autoMoveOptions(bagId) {
  ...
  cr.automove({
    nodesMatching: cr.$("node.tree[bag = \"".concat(bagId, "\"]")),
    //specify nodes that should be automoved
    reposition: 'drag',
    //specify how a node's position should be updated
    dragWith: cr.$("node.construct[id = \"".concat(bagId, "\"]")),
    //specify nodes that when dragged cause the matched nodes to move along,
    when: 'drag',
