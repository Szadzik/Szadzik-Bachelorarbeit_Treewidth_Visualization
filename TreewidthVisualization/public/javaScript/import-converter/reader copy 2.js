...
function readTdFile(file) {
    ...
    reader.onload = function(event) {
        ...
        //calculateNodeMaxGrad(); //TODO 
        resizeConstructNodes();
        mapAngles();
        setAutoMove();
        if (prevLayoutNameTd == "") { //set on truein setlayout
            setLayout("customCise", true);
        } else { //TODO should save layout on new load
    ...
    for (var lineNumber = 0; lineNumber < lines.length; lineNumber++) {
        ...
        switch (line[0]) {
            ...
            case 'b':
                ...
                if (line.length <= 2) { //<= because of empty last index like string in c++
                    let message = 'Tried to define a bag,but this is not a bag on line: ' +
                        lineNumber + '.' + '</br>' + 'Be sure to follow the bag format: ' +
                        '"<b><span style=blue:red> b </span> <span style=color:red> n </span> <span style=green:red> k" </span> </b>' +
                        ', where n is <span style=color:red> the bag number </span> and <span style=green:red> k </span> ' +
                        'a list of nodes (can be empty) of the bag <span style=color:blue> b </span>. '
                    throw new Error(message);
                }
...
function setLayout(layout, boolIsTd) {
    ...
    console.log("prelayout ", prevLayout);
    //console.log("array ", layouts[layout]);
    if (prevLayout) {
        prevLayout.stop();
    };
